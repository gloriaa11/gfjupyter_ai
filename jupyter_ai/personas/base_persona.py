...
class BasePersona(ABC, LoggingConfigurable, metaclass=ABCLoggingConfigurableMeta):
    ...
    async def stream_message(self, reply_stream: "AsyncIterator") -> None:
        ...
        print(f"[DEBUG] stream_message called, reply_stream type: {type(reply_stream)}")
        print(f"[DEBUG] reply_stream dir: {dir(reply_stream)}")
        print(f"[DEBUG] reply_stream hasattr __aiter__: {hasattr(reply_stream, '__aiter__')}")
        print(f"[DEBUG] reply_stream hasattr __anext__: {hasattr(reply_stream, '__anext__')}")
        stream_id: Optional[str] = None
        stream_interrupted = False
        try:
            self.awareness.set_local_state_field("isWriting", True)
            chunk_count = 0
            print(f"[DEBUG] Entering async for chunk in reply_stream...")
            # 手动测试 reply_stream 是否能产出 chunk
            try:
                ...
                for i in range(3):
                    try:
                        ...
                        print(f"[DEBUG] reply_stream 手动产出异常: {e}")
                        break
            except Exception as e:
                print(f"[DEBUG] reply_stream __aiter__() 异常: {e}")

            # 正常流式处理
            async for chunk in reply_stream:
                print(f"[DEBUG] Iteration start, chunk_count={chunk_count}, stream_id={stream_id}")
                chunk_count += 1
                print(f"🔍 DEBUG base_persona: Chunk {chunk_count}: {chunk}")
                if (
                    stream_id
                    and stream_id in self.message_interrupted.keys()
                    and self.message_interrupted[stream_id].is_set()
                ):
                    print(f"[DEBUG] Streaming interrupted detected for stream_id={stream_id}")
                    try:
                        await reply_stream.athrow(  # type:ignore[attr-defined]
                            GenerationInterrupted()
                        )
                    except GenerationInterrupted:
                        print(f"[DEBUG] GenerationInterrupted exception caught in athrow")
                        pass
                    stream_interrupted = True
                    break

                if not stream_id:
                    print(f"[DEBUG] stream_id is None, creating new message...")
                    stream_id = self.ychat.add_message(
                        NewMessage(body="", sender=self.id)
                    )
                    print(f"[DEBUG] New stream_id created: {stream_id}")
                    self.message_interrupted[stream_id] = asyncio.Event()
                    self.awareness.set_local_state_field("isWriting", stream_id)

                assert stream_id
                print(f"[DEBUG] Updating message for stream_id={stream_id}")
            ...
            print(f"[DEBUG] async for loop finished, chunk_count={chunk_count}")
        except ValueError as ve:
            print(f"[DEBUG] ValueError caught: {ve}")
            print(f"[DEBUG] reply_stream type: {type(reply_stream)}")
            print(f"[DEBUG] reply_stream dir: {dir(reply_stream)}")
            print(f"[DEBUG] reply_stream hasattr __aiter__: {hasattr(reply_stream, '__aiter__')}")
            print(f"[DEBUG] reply_stream hasattr __anext__: {hasattr(reply_stream, '__anext__')}")
            import traceback
            traceback.print_exc()
            self.log.error(f"Persona '{self.name}' ValueError: {ve}")
            self.log.exception(ve)
        except Exception as e:
            import traceback
            print(f"❌ DEBUG base_persona: 流式输出异常: {e}")
            traceback.print_exc()
            self.log.error(
            ...
            print(f"✅ DEBUG base_persona: 流式输出完成，共处理 {chunk_count} 个chunk (finally block)")
            self.awareness.set_local_state_field("isWriting", False)
            if stream_id:
                if stream_interrupted:
                    print(f"[DEBUG] Adding tombstone to interrupted stream_id={stream_id}")
                ...
                if stream_id in self.message_interrupted.keys():
                    print(f"[DEBUG] Deleting message_interrupted for stream_id={stream_id}")
